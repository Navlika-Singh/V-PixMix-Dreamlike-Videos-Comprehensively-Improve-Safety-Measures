...
def main_training_testing(EXP_NAME):
    ...
    #                     help='True -> only final layers, False -> whole model')
    parser.add_argument('--feature_extract', type=int, default=1,
                        help='True -> only final layers, False -> whole model')
    # parser.add_argument('--use_pretrained', default=True, type=bool,
    #                     help='True -> pretrained model, False -> from scratch')
    parser.add_argument('--use_pretrained', type=int, default=1,
                        help='True -> pretrained model, False -> from scratch')
    # parser.add_argument('--pixmix_enable', default=True, type=bool,
    #                     help='True -> pixmix data augmentation enabled, False -> pixmix data augmentation not enabled')
    parser.add_argument('--pixmix_enable', action="store_true",
                        help='True -> pixmix data augmentation enabled, False -> pixmix data augmentation not enabled')
    parser.add_argument('--mixing_path', default="/home/navlikas/videos_for_image_tasks/fractals_and_fvis", type=str,
                        help='path to mixing dataset')
    parser.add_argument('--dataset_path', default='/home/navlikas/videos_for_image_tasks/ucf101-supervised-main/Data', type=str,
                        help='dataset name')
    ...
    train_dataset, test_dataset = DATASET_GETTERS[args.dataset](args.dataset_path, args.frames_path, args.mixing_path, args.pixmix_enable, adv=False)
    print("PixMix Enabled:", args.pixmix_enable)

    model = create_model(args.arch, args.num_class, args.feature_extract, args.use_pretrained == False)
    exp_type = ""
    if args.feature_extract == True:
        exp_type = "only update the reshaped layer params"
    else:
        exp_type = "finetune the whole model"

    print("[Experiment Specs] Pretrained:{} {}".format(args.use_pretrained, exp_type))
    model.to(device)
